...
public function integer of_addannotation (eon_mobile_str_annotation astr_annotation, boolean ab_move)
...
if isnull(astr_annotation) or isnull(ab_move) then
	...
	if appeongetclienttype() = "MOBILE" then
		li_return = super:: of_addannotation(astr_annotation,ab_move)
	else
		li_return = -1
	end if
end if

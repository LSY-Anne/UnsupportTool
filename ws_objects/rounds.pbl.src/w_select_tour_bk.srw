$PBExportHeader$w_select_tour_bk.srw
forward
global type w_select_tour_bk from window
end type
type pb_calendar from picture within w_select_tour_bk
end type
type dw_shift_list from datawindow within w_select_tour_bk
end type
type st_rev_no from statictext within w_select_tour_bk
end type
type dw_tour from datawindow within w_select_tour_bk
end type
type dw_tour_group from datawindow within w_select_tour_bk
end type
type cb_cancel from commandbutton within w_select_tour_bk
end type
type cb_ok from commandbutton within w_select_tour_bk
end type
type ddlb_shift from dropdownlistbox within w_select_tour_bk
end type
type st_shift from statictext within w_select_tour_bk
end type
type st_tour_date from statictext within w_select_tour_bk
end type
type em_tour_date from editmask within w_select_tour_bk
end type
type st_tour from statictext within w_select_tour_bk
end type
type st_label from statictext within w_select_tour_bk
end type
end forward
...
em_tour_date em_tour_date
st_tour st_tour
st_label st_label
end type
global w_select_tour_bk w_select_tour_bk
...
this.em_tour_date=create em_tour_date
this.st_tour=create st_tour
this.st_label=create st_label
this.Control[]={this.pb_calendar,&
this.dw_shift_list,&
this.st_rev_no,&
this.dw_tour,&
this.dw_tour_group,&
...
OpenwithParm(w_calendar, em_tour_date.Text)
...
//		MessageBox(st_label.Text, ls_MB_String)
		...
		em_tour_date.Enabled = FALSE
		pb_calendar.Enabled = FALSE
		cb_ok.Enabled = FALSE
	ELSE
		dw_shift_list.Enabled = TRUE
		em_tour_date.Enabled = TRUE
		pb_calendar.Enabled = TRUE
		cb_ok.Enabled = TRUE
...
ls_Date = em_tour_date.Text
lb_No_Tours = (dw_tour.RowCount() <= 0)
lb_No_Date = Empty(ls_Date)

lb_Invalid_Date = NOT IsDate(ls_Date)

IF lb_Invalid_Date THEN
	ls_MB_String = f_LanguageString('THE_DATE_ENTERED_###_IS_INVALID.', 'The date entered: "###" is invalid.')
	ls_MB_String = StrTran(ls_MB_String, '###', ls_Date)
	MessageBox(f_LanguageString('WARNING', 'Warning'), ls_MB_String)
end if
lb_Invalid_Date = false

datawindowchild ldwc_temp
dw_shift_list.GetChild('shift_desc', ldwc_temp)
li_Shift_No = Integer(dw_shift_list.GetText())
li_Row = ldwc_temp.Find("shift_no = " + String(li_Shift_No), 1, ldwc_temp.RowCount())
IF ( li_Row > 0 ) THEN
	ls_Shift_Chosen = ldwc_temp.GetItemString(li_row, 'shift_desc')
END IF
...
//			integer li_Shift
			...
			li_DOW = DayNumber(Date(ls_Date))
			li_DOM = Day(Date(ls_Date))
...
//			li_Shift = Integer(MID(ls_Shift_Chosen, 1, POS(ls_Shift_Chosen, " ") - 1))
			...
			gsu_tour_parameters[gi_Current_Tour].tour_date = Date(ls_Date)
